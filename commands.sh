...
limit_tp_battery() {
   # Sets the battery min/max charge boundaries on thinkpads
   sudo modprobe tp-smapi
   echo $1 | sudo tee /sys/devices/platform/smapi/BAT0/start_charge_thresh
   echo $2 | sudo tee /sys/devices/platform/smapi/BAT0/stop_charge_thresh
}

set_ptrace() {
   # Sets the ptrace kernel module for profiling
   echo "0" | sudo tee /proc/sys/kernel/yama/ptrace_scope
}

gterm_solarize() {
   # Sets colorscheme solarized for gnome-terminal
   case "$1" in
      "dark")
